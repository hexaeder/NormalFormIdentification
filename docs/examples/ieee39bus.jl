...
begin
    ...
    SciMLBase.successful_retcode(sol) || @warn "sol was not successfull"

    @info "Run sol_nf_lin"
    prob_nf_lin = ODEProblem(nw_nf_lin, s0_nf_lin, (0.0, 15.0); add_comp_cb)
    sol_nf_lin = solve(prob_nf_lin, Rodas5P())
    SciMLBase.successful_retcode(sol_nf_lin) || @warn "sol nf was not successfull"
    ...
    # SciMLBase.successful_retcode(sol_nf_lin_legacy) || @warn "sol nf legacy was not successfull"

    @info "Run sol_dq_lin"
    prob_dq_lin = ODEProblem(nw_dq_lin, s0_dq_lin, (0.0, 15.0); add_comp_cb)
    sol_dq_lin = solve(prob_dq_lin, Rodas5P())
    SciMLBase.successful_retcode(sol_dq_lin) || @warn "sol dq was not successfull"
end;
...
let fig = Figure(; size=(800, 800))
    ...
    lines!(ax1, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax1, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax1, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax1, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
    ...
    lines!(ax2, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax2, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax2, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax2, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
           label="DQ Linearized", linewidth=2)
...
let fig = Figure(; size=(800, 1200))
    ...
    ax3 = Axis(fig[3, 1];
        title="P at Bus $src_bus (source)",
        xlabel="Time [s]",
        ylabel="Voltage Magnitude [pu]")
    lines!(ax3, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax3, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax3, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax3, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊P)).u;
           label="DQ Linearized", linewidth=2)
    axislegend(ax3; position=:rb)

    ax4 = Axis(fig[4, 1];
        title="Q at Bus $src_bus (source)",
        xlabel="Time [s]",
        ylabel="Voltage Angle [rad]")
    lines!(ax4, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax4, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax4, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax4, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊Q)).u;
           label="DQ Linearized", linewidth=2)
...
let fig = Figure(; size=(800, 1200))
    ...
    ax1 = Axis(fig[1, 1];
        ...
        xlabel="Time [s]",
        ylabel="Voltage Magnitude [pu]")
    lines!(ax1, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊u_r)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax1, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊u_r)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax1, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊u_r)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax1, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊u_r)).u;
           label="DQ Linearized", linewidth=2)
    axislegend(ax1; position=:rb)
    ...
    ax2 = Axis(fig[2, 1];
        ...
        xlabel="Time [s]",
        ylabel="Voltage Angle [rad]")
    lines!(ax2, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊u_i)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax2, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊u_i)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax2, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊u_i)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax2, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊u_i)).u;
           label="DQ Linearized", linewidth=2)
    axislegend(ax2; position=:rb)
    ...
    ax3 = Axis(fig[3, 1];
        ...
        xlabel="Time [s]",
        ylabel="Voltage Magnitude [pu]")
    lines!(ax3, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊i_r)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax3, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊i_r)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax3, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊i_r)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax3, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊i_r)).u;
           label="DQ Linearized", linewidth=2)
    axislegend(ax3; position=:rb)
    ...
    ax4 = Axis(fig[4, 1];
        ...
        xlabel="Time [s]",
        ylabel="Voltage Angle [rad]")
    lines!(ax4, ts, sol(ts; idxs=VIndex(src_bus, :busbar₊i_i)).u;
           label="Nonlinear", linewidth=2)
    lines!(ax4, ts, sol_nf_lin(ts; idxs=VIndex(src_bus, :busbar₊i_i)).u;
           label="NF Linearized", linewidth=2)
    # lines!(ax4, ts, sol_nf_lin_legacy(ts; idxs=VIndex(src_bus, :busbar₊i_i)).u;
    #        label="NF Linearized (legacy)", linewidth=2)
    lines!(ax4, ts, sol_dq_lin(ts; idxs=VIndex(src_bus, :busbar₊i_i)).u;
           label="DQ Linearized", linewidth=2)
    axislegend(ax4; position=:rb)
